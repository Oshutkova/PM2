...
class Kamera_class:
    ...
    usl_change_min = 0  # при изменении количества на заданную величину 
                        # (и усугубления ситуации) - повторное сообщение
    ...
    #УСловие превышения
    last_send_more_usl_dt = datetime.now()   # дата/время посл сообщ по условиям
    last_send_more_usl_cnt = 0   # количество людей в последнем сообщении
    last_send_more_norm_dt = datetime.now()  # дата/время посл сообщ по нормализации
    last_send_more_norm_cnt = 0  # кол-во людей в последнем сообщении по нормал
    # Условние снижения
    last_send_less_usl_dt = datetime.now()   # дата/время посл сообщ по условиям
    last_send_less_usl_cnt = 0   # количество людей в последнем сообщении
    last_send_less_norm_dt = datetime.now()  # дата/время посл сообщ по нормализации
    last_send_less_norm_cnt = 0  # кол-во людей в последнем сообщении по нормал
 
    b_add_mess_usl_more = 0      # признак - создать сообщение на выполнение для условия превышения
    b_add_mess_usl_more_norm = 0     # признак - созддать сообщение о нормализации для условия превышения
    b_add_mess_usl_less = 0      # признак - создать сообщение на выполнение для условия на снижение
    b_add_mess_usl_less_norm = 0     # признак - созддать сообщение о нормализации для условия на снижение
...
def get_spisok_kamer(conn):
    ...
    cur = conn.cursor()
    sql = """ 
        select C.id, C.name, C.url, C.usl_send_less, C.usl_send_more, C.usl_change_min,
               C.usl_norm_less, C.usl_norm_more,
               t_usl_more_last.date_time last_send_more_usl_dt,  t_usl_more_last.cnt_people last_send_more_usl_cnt,
               t_more_norm_last.date_time last_send_more_norm_dt,  t_more_norm_last.cnt_people last_send_more_norm_cnt,
               t_usl_less_last.date_time last_send_less_usl_dt,  t_usl_less_last.cnt_people last_send_less_usl_cnt,
               t_less_norm_last.date_time last_send_less_norm_dt,  t_less_norm_last.cnt_people last_send_less_norm_cnt
        ...
        left join (select tc.id_calc_people_camera, max(id) id_max
                           ...
                           group by tc.id_calc_people_camera) t_usl_more_last_id on t_usl_more_last_id.id_calc_people_camera = C.id
          left join calc_people_camera_cnt_people t_usl_more_last on t_usl_more_last.id = t_usl_more_last_id.id_max
        left join (select tc.id_calc_people_camera, max(id) id_max
                           from calc_people_camera_cnt_people tc
                           where tc.b_add_mess_usl_more_norm = True
                           group by tc.id_calc_people_camera) t_usl_more_norm_id on t_usl_more_norm_id.id_calc_people_camera = C.id
        left join calc_people_camera_cnt_people t_more_norm_last on t_more_norm_last.id = t_usl_more_norm_id.id_max   
                  
              left join (select tc.id_calc_people_camera, max(id) id_max
                           from calc_people_camera_cnt_people tc
                           where tc.b_add_mess_usl_less = True
                           group by tc.id_calc_people_camera) t_usl_less_last_id on t_usl_less_last_id.id_calc_people_camera = C.id
          left join calc_people_camera_cnt_people t_usl_less_last on t_usl_less_last.id = t_usl_less_last_id.id_max
        left join (select tc.id_calc_people_camera, max(id) id_max
                           from calc_people_camera_cnt_people tc
                           where tc.b_add_mess_usl_less_norm = True
                           group by tc.id_calc_people_camera) t_usl_less_norm_id on t_usl_less_norm_id.id_calc_people_camera = C.id
          left join calc_people_camera_cnt_people t_less_norm_last on t_less_norm_last.id = t_usl_less_norm_id.id_max    
        """
    ...
    for i, kam in enumerate(spisok_kamer):
        ...
        if kam.usl_send_more > 0 and kam.cnt_people > kam.usl_send_more and \
           (kam.last_send_more_usl_dt == datetime(1900, 1, 1) or \
            (datetime.now() - kam.last_send_more_usl_dt).total_seconds() > min_inrerval_sec or \
            (kam.last_send_more_norm_dt != datetime(1900, 1, 1) and kam.last_send_more_norm_dt > kam.last_send_more_usl_dt) or \
        ...
        if kam.usl_send_less > 0 and kam.cnt_people < kam.usl_send_less and \
           (kam.last_send_less_usl_dt == datetime(1900, 1, 1) or \
            (datetime.now() - kam.last_send_less_usl_dt).total_seconds() >= min_inrerval_sec or \
            (kam.last_send_less_norm_dt != datetime(1900, 1, 1) and kam.last_send_less_norm_dt > kam.last_send_less_usl_dt) or \
